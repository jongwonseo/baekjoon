...
dp = [[0 for _ in range(sum(C)+1)] for _ in range(N+1)] #냅색알고리즘이 실행될 dp 메모리 크기 들어감
result = sum(C) #열의 최댓값
...
for i in range(1, N+1):
    ...
    for j in range(1, sum(C) + 1): #i까지 앱중에서 j코스트로 얻을 수 있는 최대 바이트
        ...
        if dp[i][j] >= M: #조건이 충족된다면
          result = min(result, j) #더 작은 cost값(j)으로 갱신
